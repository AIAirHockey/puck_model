...
def calculate_restitution():
    ...
    # Load the puck data
    data = pd.read_csv(f"{PROJECT_PATH}/data/position_15.csv")
    
    # Segment the data to identify wall collisions
    segments = puck_data_2.segment_puck_trajectory(data['x'], data['y'], data['dt'], velocity_threshold=0.1, buffer=1)
...
def main():
    ...
    restitution_values = calculate_restitution()
